...
def SpeedTest():
    ...
    return [current_date, current_time, float(res["ping"]), float(humanbytes(res["download"])), float(humanbytes(res["upload"])), float(res["server"]["id"]), str(res["server"]["name"]+"/"+res["server"]["sponsor"]), str(res["server"]["country"]) ]
